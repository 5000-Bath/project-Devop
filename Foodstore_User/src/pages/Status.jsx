...
import { useState, useEffect } from 'react';
import '../component/Status.css';
import { getOrderById } from '../mockdb';     // 👈 ดึงจาก mockdb
...
export default function Status() {
  ...
  const total = order
    ? order.items.reduce((s, it) => s + Number(it.price) * Number(it.qty ?? it.quantity ?? 1), 0)
              ...
              <div>
                {order.items.map((it, i) => (
                  <div key={i} className="item-row">
                    {it.img ? <img src={it.img} alt={it.name} className="item-thumb" /> : <div className="item-thumb" style={{background:'#f3f4f6'}} />}
                    <div>
                      <div className="item-name">{it.name}</div>
                      <div className="item-price">
                        {Number(it.price)} {order.currency} × {Number(it.qty ?? it.quantity ?? 1)}
                      </div>
                    </div>
                  </div>
                ))}
