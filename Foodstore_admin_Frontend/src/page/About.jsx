...
const getCategory = (item) =>
  (item?.category && (item.category.name ?? item.category.title ?? item.category)) ??
  item?.categoryName ??
  item?.type ??
  "-";
...
export default function About() {
  ...
  const handleUpdate = async () => {
    try {
      ...
      setMenuItems((prev) => prev.map((p) => (p.id === updated.id ? updated : p)));
      setSelectedItem(null);
            ...
            {menuItems
              .filter((item) => {
                ...
                return (
                  ...
                  String(getCategory(item)).toLowerCase().includes(q)
                );
              })
              .map((item) => (
                <tr
                  ...
                  onClick={() => {
                    ...
                    setEditData({
                      ...
                      price: item.price || 0,
                      stock: item.stock || 0,
                      category: getCategory(item) !== "-" ? getCategory(item) : "", 
                    });
                  }}
                >
                  ...
                  <td style={{ padding: "12px 8px" }}>{getCategory(item)}</td>
      ...
      {selectedItem && (
        ...
        >
          ...
          >
            ...
            <h3 style={{ marginBottom: 16, fontSize: 20 }}>{selectedItem.name}</h3>
            ...
            <label style={{ display: "block", marginBottom: 6, fontWeight: 500 }}>Price (บาท)</label>
            <input
              type="number"
